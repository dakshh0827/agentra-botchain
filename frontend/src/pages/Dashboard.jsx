...
export default function Dashboard() {
  ...
  const metricCards = [
    { label: 'TOTAL REVENUE', value: `${totalRevenue.toFixed(4)} token`, color: 'green', icon: DollarSign, sublabel: 'All time earnings' },
            ...
            <div className="glass-card-landing rounded-xl p-5 sm:p-6 h-full">
              <div className="flex items-center justify-between mb-6">
                <div>
                  ...
                  <p className="text-[var(--color-text-dim)] text-sm font-mono tracking-wider mt-0.5">TOKEN EARNINGS OVER TIME</p>
                </div>
              </div>
              {revenueData.length > 0 ? (
                <ResponsiveContainer width="100%" height={200}>
                  <AreaChart data={revenueData}>
                    ...
                    <Area type="monotone" dataKey="eth" stroke="#a855f7" strokeWidth={2} fill="rgba(168,85,247,0.12)" name="token" />
                  </AreaChart>
                </ResponsiveContainer>
              ) : (
            ...
            {agentPerf.length > 0 ? (
              <ResponsiveContainer width="100%" height={200}>
                <BarChart data={agentPerf} barGap={6}>
                  ...
                  <Bar dataKey="revenue" fill="rgba(52,211,153,0.4)" stroke="#34d399" strokeWidth={1} radius={[4, 4, 0, 0]} name="Revenue (token)" />
                </BarChart>
              </ResponsiveContainer>
            ) : (
              <div className="flex flex-col items-center justify-center h-[200px] text-center">
